    ...
    while True:
        ...
        if file.query.exact_matches:
            choices = [
                dict(name=str(match),
                     value=match)
                for match in file.query.exact_matches
            ]
        ...
        if file.query.fuzzy_matches:
            choices = [
                dict(name=f'{str(match)} ({match.fuzzy_match_score})',
                     value=match)
...
def prompt_send_revise_skip(file: File, source: Union[MediaName, MediaNameParse, QueryResult]) -> bool:
    ...
    choices = ['Send to Media Library', 'Manually Enter Information', 'Skip File']
    question = dict(
        type='list',
        name='send',
        message='Choose to send, revise, or skip',
        choices=choices,
        default='Send to Media Library'
    )
...
def prompt_list_of_matches(file: File, match_type: str, choices: list):
    ...
    questions = [
        dict(
            type='list',
            name='match',
            message=f'Choose a {match_type} match for {file.filepath.name}',
            choices=choices
        )
    ]
    answers = PyInquirer.prompt(questions)
